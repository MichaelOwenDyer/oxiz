...
//!
//! These mirror the z3-python top-level API:
//!   `StringVal(s)`, `Concat(a, b)`, `Length(s)`, `Contains(s, sub)`,
//!   `PrefixOf(pre, s)`, `SuffixOf(suf, s)`,
//!   `FPSort(eb, sb)`, `FPVal(sign, exp, sig, sort)`,
//!   `fp_add(rm, a, b)`, `fp_sub(...)`, `fp_mul(...)`, `fp_div(...)`,
//!   `FPRoundingMode` (sort object),
//!   `ArraySort(index_sort, elem_sort)`,
//!   `ForAll(vars, body)`, `Exists(vars, body)`
...
/// Example::
///
///     fp16 = oxiz.FPSort(5, 11, tm)   # IEEE 754 half-precision
///     fp32 = oxiz.FPSort(8, 24, tm)   # IEEE 754 single-precision
#[pyfunction]
...
/// Example::
///
///     arr_sort = oxiz.ArraySort(oxiz.IntSort(tm), oxiz.IntSort(tm), tm)
#[pyfunction]
    ...
    let (eb, sb) = match (sort.eb, sort.sb) {
        ...
        _ => {
            return Err(PyValueError::new_err(
                "FPVal: the 'sort' argument must be an FPSort (created with oxiz.FPSort(eb, sb, tm))",
            ));
        }
    };
    let mut inner = tm.inner.borrow_mut();
...
///     fml = oxiz.ForAll([("x", "Int")], body, ctx.tm)
#[pyfunction]
...
///     x = ctx.int_const("x")
///     body = x > ctx.int_val(0)
///     fml = oxiz.Exists([("x", "Int")], body, ctx.tm)
#[pyfunction]
